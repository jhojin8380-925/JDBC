...
<%@ page import="article.ArticleDAO, article.ArticleDTO, java.util.List"%>
...
<body>
	...
	<%-- 회원 등록 페이지로 이동하는 링크 --%>
	<a href="articleInsert" class="btn-insert">게시글 등록</a>
	...
	<table>
		...
		<tbody>
			...
			%>
			<tr>
				<td><%=dto.getArticleId()%></td>
				<td><%=dto.getArticleTitle()%></td>
				<td><%=dto.getArticleBody()%></td>
				<%-- 수정 링크 : 클릭하면 memberId를 url에 수정 서블릿으로 이동 --%>
				<td><a href="articleUpdate?articleId=<%=dto.getArticleId()%>">수정</a>
				</td>
				<%-- 삭제 링크 :  --%>
				<td><a href="articleDelete?articleId=<%=dto.getArticleId()%>"
					onclick="return confirm(&quot; <%= dto.getArticleTitle() %>을 삭제하시겠습니까?&quot;)">삭제</a></td>
			</tr>
			<%
			}
			%>
