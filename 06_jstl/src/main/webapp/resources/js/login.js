...
function getCookie(name){
	...
	// ex) "saveName=홍길동; theme=dark; lang=ko"
	// split(";") : ";" 기준으로 잘라 배열로 만든다
	let cookies = document.cookie.split(";");
	
	for(let i = 0; i < cookies.length; i++){
		// trim() : 앞뒤 공백 제거(쿠키 구분자 ";" 뒤에 공백이 붙는 경우가 있다)
		...
		if(cookie.startsWith(name+"=")){
			...
			// decodeURIComponent() : 서버에서 URLEncoder로 인코딩된 한글을
			// 다시 원래 글자로 복원
			return decodeURIComponent(encodedValue);
		}
	}
	// 이름이 일치하는 쿠키가 없으면 null을 반환
	return null;
