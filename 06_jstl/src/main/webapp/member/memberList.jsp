...
<body>
	...
	<!-- 세션에 있는 회원의 이름을 화면에 출력 -->
		<span class = "welcome-msg">
			...
			<%-- 세션에서 loginMember를 꺼내고 getMemberName()자동 호출 --%>
			<strong>${loginMember.memberName}</strong>님 환영합니다!
		</span>
		<!-- 로그아웃 : /logout 서블릿으로 이동 -> 세션 invalidate 
			-> /login으로 redirect -->
	...
	<p>
		<%-- 전체 회원 수 : <strong><%= memberList.size()%></strong>명 --%>
	...
	<table>
		...
		<tbody>
			...
			변경 : c:forEach
				- var = "dto" : 반복 변수 이름
				- items = ${memberList} : 
						Controller가 setAttribute("meberList", list)로 담은 리스트
				- varStatus="status" : 순번정보(status.count = 1부터 시작)
			 --%>
			<c:forEach var="dto" items="${memberList}" varStatus="status">
				<tr>
					...
					<td>
						...
						onclick="location.href='${pageContext.request.contextPath}/memberUpdate?memberId=${dto.memberId}'">               
						수정</button>
						<button class="btn btn-delete" 
						onclick="location.href='${pageContext.request.contextPath}/memberDelete?memberId=${dto.memberId}'">
						삭제</button>
					</td>
				</tr>
			</c:forEach>
